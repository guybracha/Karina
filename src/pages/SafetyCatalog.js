...
export default function SafetyCatalog() {
  ...
  }, [query, color, size, sort, page, params, setParams]);
  ...
  const filtered = useMemo(() => {
    ...
    return PRODUCTS.filter((p) => {
      // רק מוצרי בטיחות
      if (p.category !== "safety") return false;
  ...
  useEffect(() => { setPage(1); }, [query, color, size, sort]);

  function clearAll() {
    setQuery(""); setColor(""); setSize(""); setSort("popular"); setPage(1);
  }
    ...
    <div className="catalog-page-bg">
      <div className="container py-4" dir="rtl">
        ...
        <div className="d-flex flex-column flex-md-row align-items-md-center justify-content-between gap-2 mb-3">
          <h1 className="m-0">מוצרי בטיחות</h1>
          <div className="text-muted">
            נמצאו <strong>{sorted.length}</strong> פריטים
          </div>
        </div>

        {/* תמונת רקע לקטגוריה */}
        <div className="category-hero mb-3" style={{ backgroundImage: `url(${safety})`, height: '200px', backgroundSize: 'cover', backgroundPosition: 'center', borderRadius: '8px' }}>
          <div style={{ background: 'rgba(0,0,0,0.5)', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: '8px' }}>
            <div style={{ color: 'white', textAlign: 'center' }}>
              <h2>🦺 ציוד בטיחות מקצועי</h2>
              <p>ווסטים זוהרים, קסדות ואביזרי בטיחות</p>
            </div>
          </div>
        </div>
        ...
        {paged.length > 0 ? (
          ...
          <div className="text-center empty-state">
            ...
            <p className="text-muted mb-3">נסו לאפס מסננים או לבחור מסננים אחרים.</p>
